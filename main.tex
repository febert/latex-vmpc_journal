...
\IEEEcompsocthanksitem  Frederik Ebert*, Chelsea Finn*, Annie Xie, Sudeep Dasari, Alex Lee, Sergey Levine}% <-this % stops an unwanted space
...
\begin{abstract}
Deep reinforcement learning algorithms have enabled learning of complex skills and mastery of complex games. Unlike prior works that have focused on mastery of complex, specialized behaviors in limited, closed-world environments, we consider a reinforcement learning problem setting with more simple skills but with substantial diversity, raw observations, and sparse rewards. As a result, rewards are simply not enough supervision to be learned from alone. Instead we propose to learn from the observations themselves, learning predictive models of future observations conditioned on the agent's future actions, and then choosing actions by planning under this observation model. We show that we can learn observation models effectively from real videos of a robot's experience, that such models can handle diverse datasets with hundreds of objects, and that they enable the robot to accomplish a wide range of object manipulation goals, even when faced with novel objects.
\todo{Page limit is 12!!}
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
Deep Reinforcement Learning, Observations Models, Video Prediction, Robotic Manipulation, Model Predictive Control
\end{IEEEkeywords}}
