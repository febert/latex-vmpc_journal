...
%%SL.11.21: I still think this needs more motivation -- saying we do this just b/c it's easy to evaluate is rather unsatisfying. Maybe we can say something like: This type of objective is general, in that it can specify any object relocation task, and very easy for a person to specify.
...
Given a distribution over pixel positions $P_0$ at time $t = 0$, our model predicts distributions over its positions $P_t$ at time $t \in \{ 1, \dots, T \}$. One way of defining the cost per time-step $c_t$ is by using the expected euclidean
%%SL.11.21: euclidean should be capitalized
...
%%SL.11.21: This is a bit confusing -- you say initial location at each time step. Initial relative to what? I think what you are implying is that you need to know the position at the beginning of each planning stage, but this is not at all clear since the reader won't be thinking about MPC at this point. Maybe just say you need to know the current location?
...
%%SL.11.21: This is a good discussion to have, but it feels really out of place here. And it's kind of floating in a vacuum without context. What I might suggest is to shorten it, and maybe move it somewhere else (earlier?)
