...
\noindent \textbf{Quantitative Evaluation} We define a set of tasks where the robot is required to move objects in its environment from a starting state to a goal configuration, and performance is evaluated by measuring the distance between the final object position and goal position. 
...
\begin{enumerate}
	...
	\item [\ref{subsec:multi_task_bench}]
\end{enumerate}
...
\todo{explain motivation: we wan to assess the ability of our algorithm to handle a wide range of tasks without problem-specific parameter tuning.}
\todo{put in example trajecotries!}
